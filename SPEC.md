...
- set an optional display name and per-seller contact link
...
- `contact_url` nullable, per-seller contact link template
...
- contact URL is generated from the item seller's `contact_url` when set, falling back to `PUBLIC_CONTACT_URL_TEMPLATE`
...
- the owner account is seeded from `ADMIN_EMAIL`/`ADMIN_USERNAME` and `ADMIN_PASSWORD_HASH` on first login; the database becomes the source of truth afterwards
